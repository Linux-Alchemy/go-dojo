...
- Last scope asked: ch1–4, difficulty 2–3, low-key/low-effort — 2026-09-13 (session 2, carried from 09-12)
- Reps completed: 5 (session 1: 001–003, all pass-with-notes at 2/10; session 2: 004 clean, 005 pass-notes)
- Next: rep 006 in `main.go`, **not yet attempted** (COMPLETE 3/10, splitPot multiple-returns + zero-members guard). Resume there. Session 2 (2026-09-13) asked for low-key, low-effort: stay ch1–4 at 2–3.
...
|---|---|---|---|---|
| short-decl (`:=` vs `=` vs `var`) | 1 | shaky | 001 | asked why `var x T :=` and `x T :=` fail; python type-hint reflex |
| zero-values | 1 | unseen | – | |
| type-conversion (int↔float, truncation) | 1 | shaky | 005 | asked why float64(a/b) gave 11.00; convert inputs, not result |
| int-division | 1 | shaky | 005 | predicted right after explanation; Println drops .0 on floats was news |
...
| grouped-params | 4 | unseen | – | |
| multiple-returns | 4 | unseen | – | |
| blank-identifier | 4 | unseen | – | |
| named-returns | 4 | unseen | – | |
| naked-return | 4 | unseen | – | |
| pass-by-value | 4 | unseen | – | |
| guard-clauses | 4 | solid | 004 | spotted `<` vs `<=` boundary in ordered guards |
...
- **int division / conversion timing** (seen 09-12 and 09-13): expects `/` to keep the fraction; `float64(a / b)` looked like it should rescue it. Keep seeding PREDICTs where the conversion comes too late.
