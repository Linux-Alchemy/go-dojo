...
- Reps completed: 22 (session 9: 021–022 pass-notes; session 8: 017–020 all pass-notes; session 7: 013 pass-notes 5/6 blind, 014 pass-notes after one nudge, 015 clean, 016 clean)
- Session 8 (2026-09-21) scope: Matt asked for a few rounds of **ch5 structs only at 2–3** after 017.
- Next: rep 023 in `main.go`, not yet attempted (TRANSLATE 3/10, pub booth availability; anonymous-structs). Session 9 closed 2026-09-22; resume here with ch5 structs only at 2–3.
...
| typed-value-validation | 5 | shaky | 2026-09-19 | – | – | Recovered historical collection COMPLETE: correct without nudges, redundant boolean-return branch; one shape only. See lessons/ch5_typed_validation.md and archive/2026-09-19-import/. Outside current rep numbering. |
| struct-zero-values | 5 | shaky | 2026-09-21 | – | – | 019: omitted everything on the mild keg correctly, but wrote `chilled: false` out on the stout (asked not to write zero values). Recovered 2026-09-19 PREDICT: correct values, but bool-zero reasoning needed clarification (false directly, not numeric conversion); later rating/date retained. |
| nested-structs | 5 | shaky | 2026-09-21 | – | – | 020: PREDICT, read `a.stats.hp` / `b.stats.level` through the nested struct correctly on every line he attempted. Recovered 2026-09-19 nested-stock FIX was clean; collection COMPLETE had an idiom note. Retain shaky: no second clean shape established. |
...
| methods-value-receiver | 5 | shaky | 2026-09-22 | – | – | 021 COMPLETE: multiple nudges on method name vs local variable, receiver field access and arithmetic. 022 FIX: code corrected unaided, output/gofmt/vet clean; explanation clarified zero-valued literal vs empty struct and whole-struct receiver copy vs just fuel. Correctly traced 6 * 3; revisit -> shaky, independent receiver-copy explanation still pending. |
...
## Struggle Patterns
- **Type guarantees versus value validation** (recovered learner report, 2026-09-19): tried checking whether concrete string fields might hold ints or bools; then identified that Go already enforces the types. Drill values forbidden by the task, allowing legitimate zero/false values. See `lessons/ch5_typed_validation.md`.
- Watch: **zero-valued structs versus absent fields** (2026-09-19 historical PREDICT and 2026-09-22 rep 022): described bool zero as numeric 0 converting to false, and a zero-valued lantern as an empty struct. Clarified direct false and a new struct with its fields present; independently retest the reasoning.
...
| 021 | COMPLETE | 3 | methods-value-receiver | pass, notes | – | Output 5/5; vet clean; multiple nudges on method name vs local variable, receiver field access and surcharge arithmetic. Redundant == true; gofmt extra blank line. Tag revisit. |
| 022 | FIX | 3 | methods-value-receiver | pass, notes | – | Code clean first check, output 2/2; named zero-valued literal as empty struct, then correctly traced receiver fuel=6 after clarification. Whole-struct copy clarified; tag shaky. |
...
- 2026-09-22 (session 9, closed): 2 reps, avg 3.0, ch5 structs only at 2–3. 021 pass-notes after multiple nudges; 022 pass-notes (clean unaided code fix, explanation clarified). methods-value-receiver revisit -> shaky. Paused by Matt with rep 023 TRANSLATE 3/10 (anonymous-structs) loaded and unattempted; resume there. Session note appended to SESSIONS.md.
- 2026-09-22 (historical reconciliation, no new reps): recovered the 2026-09-19 Boot.dev lesson, three verbatim exercises under archive/2026-09-19-import/, and session evidence. Original 013–015 identifiers are historical only; active numbering, 22 completed, rep 023 pending, current scope and newer ratings/dates preserved. Added typed-value-validation as shaky from historical evidence.
